...
def RaidersMarch():
    x0 = ["B3/3", "C4/8", "D4/4", "G4/1", "R/4" ]
    x1 = ["A3/3", "B3/8", "C4/1", "R/4"]
    x2 = ["D4/3", "E4/8", "F#4/4", "C5/1", "R/4"]
    x3 = ["E4/3", "F#4/8", "G4/2", "A4/2", "B4/2"]
    x4 = ["A4/3", "B4/8", "C5/1", "R/4"]
    x5 = ["D4/3", "D4/8", "B4/2", "A4/3", "D4/8", "B4/2"]
    x6 = ["A4/3", "D4/8", "B4/2", "A4/3", "D4/8", "B4/4", "A4/4" ]

    # Combining the segments into a test list
    test = x0+x1+x2+x3+x0+x4+x5+x6
    return test
async def subtask():
    print('DaKiwi')
...
async def subtask2():
    for count in range(1):
        await wait(1)
        await prime_hub.speaker.play_notes(RaidersMarch())
